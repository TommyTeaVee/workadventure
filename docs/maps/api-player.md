...
of Qtune, you can define tags related to the user in the [administration panel](https://workadventu.re/admin-guide/manage-members).
...
The token is generated by the administration panel linked to Qtune. The token is a string and is depending on your implementation of the administration panel.
...
The token can be used as a way to authenticate a Qtune player and ensure he is indeed in the map and authorized to upload a logo.
...
- Transient variables disappear as soon as the connection to the room is lost. So if you
  ...
  if you simply close Qtune and come back later, the transient player variable value
...
player variables. So far, Qtune SAAS version has no maximum TTL set. However, please note you should
...
the Qtune server. If the player is not logged, the player variables are stored in local storage (so in the
player's browser).
...
You can be connected several times with the same user to Qtune (and Qtune
will not complain about it, this is by design).
Open another tab, connect again to Qtune and you will be connected to Qtune
twice with the same user. We will call those users connected several times to Qtune
**brothers**.
